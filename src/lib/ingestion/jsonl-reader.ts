...
export async function countNewlinesUpTo(
  filePath: string,
  byteOffset: number
): Promise<number> {
  if (byteOffset <= 0) {
...
async function isOffsetAtLineBoundary(
  filePath: string,
  byteOffset: number
): Promise<boolean> {
  if (byteOffset <= 0) {
...
export async function readJsonlIncremental(
  ...
  const lineNumberBase =
    options?.lineNumberBase ?? (startOffset > 0 ? await countNewlinesUpTo(filePath, startOffset) : 0);
