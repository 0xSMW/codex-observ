...
function statusFromText(value: string): ToolCallEventType | null {
  ...
  if (
    ['ok', 'success', 'succeeded', 'complete', 'completed', 'done', 'finished', 'exit'].includes(
      normalized
    )
  ) {
    return 'exit'
...
function statusFromLine(line: string): ToolCallEventType | null {
  ...
  if (lower.includes('completed') || lower.includes('finished') || lower.includes('success'))
    return 'exit'
  if (lower.includes('started') || lower.includes('running') || lower.includes('pending'))
    return 'start'
  return null
}
