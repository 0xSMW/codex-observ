...
export function ActivityHeatmap({ year, data }: { year: number; data: ActivityResponse }) {
  const start = startOfYear(new Date(year, 0, 1))
  const end = endOfYear(new Date(year, 0, 1))
  const startGrid = startOfWeek(start, { weekStartsOn: 0 })
  const totalDays = differenceInCalendarDays(end, startGrid) + 1
  const cells = Array.from({ length: totalDays }).map((_, index) => addDays(startGrid, index))
          ...
          return (
            <Tooltip key={key} delayDuration={0}>
              ...
              <TooltipContent side="top" className="text-xs">
                ...
                <div className="text-muted-foreground">{formatCompactNumber(value)} tokens</div>
              </TooltipContent>
            </Tooltip>
          )
        })}
