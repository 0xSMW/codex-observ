...
import { getWorktrees } from '@/lib/metrics/worktrees'
...
export async function GET(request: Request) {
  try {
    ...
    const cacheKey = `worktrees:metrics:${url.searchParams.toString()}`
    ...
    console.error('worktrees:metrics failed', error)
    return jsonError('Failed to load worktree metrics', 'internal_error', 500)
  }
}
