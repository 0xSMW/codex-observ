...
console.log('\n--- Model Calls (Top 5) ---')
console.log(
  query('SELECT id, ts, model, total_tokens, duration_ms FROM model_call ORDER BY ts DESC LIMIT 5')
)

console.log('\n--- Tool Calls (Top 5) ---')
console.log(
  query('SELECT id, start_ts, duration_ms, status FROM tool_call ORDER BY start_ts DESC LIMIT 5')
)
